...
It can also be created from named components. In that case, `major`, `minor`
and `patch` are *mandatory*, and must be integers:
...
| <a id="semver.compare-_parse"></a>_parse |  **[INTERNAL]** Parser method used to convert strings to SemVer structs.   |  `<function _parse from //version:semver.bzl>` |
